...
namespace Projectile_Motion_Project.Controls
{
    public class Button : Component
    {
        ...
        public void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            ...
            {
                var x = (Rectangle.X + (Rectangle.Width / 2)) - (_font.MeasureString.X(Text) / 2);
                var y = (Rectangle.Y + (Rectangle.Height / 2)) - (_font.MeasureString.Y(Text) / 2);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            var mouseRectangle = new Rectangle(_currentMouse.X, _currentMouseState.Y, 1, 1);

            _isHovering = false;

            if(Mouse.Rectangle.Intersects(Rectangle))
            {
                _isHovering = true;
